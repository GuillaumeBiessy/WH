...
-   $F(Y,\Omega, \Theta) = \underset{i = 1}{\overset{n_x}{\sum}}\underset{j = 1}{\overset{n_z}{\sum}} \Omega_{i,j}(Y_{i,j} - \Theta_{i,j})^2$ is a fidelity criterion

-   $R(\Theta,\lambda,q) = \lambda_x \underset{j = 1}{\overset{n_z}{\sum}}\underset{i = 1}{\overset{n_x - q_x}{\sum}} (\Delta^{q_x}\Theta_{\bullet,j})_i^2 + \lambda_z \underset{i = 1}{\overset{n_x}{\sum}}\underset{j = 1}{\overset{n_z - q_z}{\sum}} (\Delta^{q_z}\Theta_{i,\bullet})_j^2$ is a regularity criterion which may be decomposed as the sum of:
...
The `WH` package features two main functions `WH_1d` and `WH_2d` corresponding to the one-dimension and two-dimension cases respectively. There are only two arguments for those functions:
...
Additional arguments are described in the documentations of those functions.
...
## Confidence / credibility intervals {#cred}
...
In the cas of 0 weights, the matrix $W^{- 1}$ is not properly defined. By defining $w_{*}$ the subvector of stricly positive weights and $y_{*}$ the associated observation vector and noting $W_{*} = \text{Diag}(w_{*})$, the previous quantity may be rewritten:
...
and the problem may now be expressed properly in terms of $y_{*}$ and $w_{*}$. Nevertheless to keep things simple we stick with $y$ and $w$ and consider that $W^{- 1}$ contains infinite values associated with initial 0 weights in $w$.
...
$$\hat{\sigma}^2 = \frac{\|\sqrt{W}(y - \hat{y})\|^2}{n_* - \text{edf}}$$ where $n_*$ corresponds to the number of non 0 weights and $\text{edf} = \text{tr}(H) = \text{tr}[(W + P_\lambda)^{- 1}W]$.
...
<!-- # How does the package handle large matrix with a lot of observations ? -->

<!-- In the two-dimension case, while working for example on disability or long-term care survival rates, the problem dimension can easily becomes too large to handle : -->

<!-- * For the french disability public scheme, temporary disability coverage extends from ages 18 to 66 and may last for 36 months, meaning the monthly data will be stored in a $49\times36$ matrix. That is 1,764 data points ! -->

<!-- * For the french disability public scheme, permanent disability coverage extends from ages 20 to 61 and may last until age 61, meaning the data will be in a $42\times42$ matrix. That is also 1,764 data points ! -->

<!-- * For french long-term care covers, while most LTC claims occurs at old ages, the covers extends from age of subscribing until death. The annuity also last until death. Assuming that claims occur from age 50 until age 109 and may last 30 years, that is still 1,800 data points. -->

<!-- # FAQ -->

<!-- ## Should I use WH smoothing to create prospective mortality tables ? -->

<!-- Smoothing methods rely on the assumption that the underlying law that one wants to estimate is smooth. In the [Confidence / credibility intervals subsection]{cred} this assumption is even formally introduced as an *a priori* on the parameter distribution. Period mortality has a lot of yearly mortality caused by both climatic (heatwaves, cold winters) and epidemiologic (seasonal flue, pandemics) phenomena. Should smoothing be used in that case, then this year-to-year volatily will be treated as a smooth but extremely volatile trend in the data which will result in extremly volatile predictions in the forecasting step. -->
