...
By defining $w_\ast$ the subvector of stricly positive weights and
$y_\ast$ the associated observation vector and noting
$W_\ast = \text{Diag}(w_\ast)$, the previous quantity may be rewritten:
...
and the problem may now be expressed properly in terms of $y_\ast$ and
$w_\ast$. Nevertheless to keep things simple we stick with $y$ and $w$
